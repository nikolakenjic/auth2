...
import {createEmptyExperience} from '@/app/lib/resume/resume.utils';
...
export function ExperienceSection({items, onChange}: ExperienceSectionProps) {
    ...
    const addItem = () => {
        const newItems = [...items, createEmptyExperience()];
        onChange(newItems);
    ...
    const removeBullet = (itemIndex: number, bulletIndex: number) => {
        ...
        updated[itemIndex] = {
            ...updated[itemIndex],
            bullets: updated[itemIndex].bullets.filter(
                (_, i) => i !== bulletIndex,
            ),
        };
        onChange(updated);
